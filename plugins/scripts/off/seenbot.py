...
inifile = os.path.join(xchatdir,"plugins/settings/seenbot.ini")
...
def load_vars():
    ...
    try:
        ...
        for item in config.items("main"):
            ...
            option["locallimit"] c= config.getint("main", "locallimit")
    ...
    try:
        seen = shelve.open(os.path.join(xchatdir, "plugins/settings/seenbot.db"),'c')
...
def on_text(word, word_eol, userdata):
    ...
    if option["service"] == True and trigger[0] == "!seen" and triggerchannel not in option["notin"]:
	    ...
	    destination.command("say " + "i dont know who " + trigger[1] + " is")
...
def command_seen(word, word_eol, userdata):
    ...
    if seen.has_key(key):
    	...
    	print color["blue"], "I dont know who " + key + " is"
...
def command_seendump(word, word_eol, userdata):
    try:
        ...
        seendump.close
        print color["blue"], len(seen), "records in the seen db"
        print color["dgreen"], "Dump complete results in", os.path.join(xchatdir,"plugins/settings/seenbot.txt")
