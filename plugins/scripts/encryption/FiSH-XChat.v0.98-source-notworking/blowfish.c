...
void blowfish_init(u_8bit_t * key, int keybytes, BF_CTX)
{
  int i, j;
...
void decrypt_key(char *theData)
{
	int i;
	unsigned char theContactKey[500];

	i=strlen(theData);
	if(i >= sizeof(theContactKey)) return;
...
void encrypt_key(char *theData)
{
	int i;
	unsigned char theContactKey[100];

	strncpy(theContactKey, theData, 80);
	theContactKey[80]='\0';

	i=strlen(theContactKey);
