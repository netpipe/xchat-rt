...
sub check_message {
	...
	return EAT_NONE unless ( $_[0][1] =~ /^\Q$PREFIX/ ); #only continue if starts with $PREFIX
...
sub slappy_command {
	...
	unless ($config->{triggers}{$trigger} && $config->{triggers}{$trigger}{command}) {
		my @validkeys = keys %{ $config->{triggers} };
		# TODO: add check for command here
		my $message2 = "Type \cB$_[0][1]\cB is not valid. Valid types: " . valid_types();
...
sub load_conf {
	...
	$config->{perunit} = 0;
	$config->{timeunit} = 60;
	$config->{objects} = "$config->{dir}/plugins/settings/randobjects.txt";

	my $trigger;
	my $message;

	open (DATA, '<', $config->{dir}.'/plugins/settings/randslappy.conf') or do {
		unless (-e $config->{dir}.'/plugins/settings/randslappy.conf') {
			$message = 'Configuration file does not exist. Please create randslappy.conf in the configuration directory.';
		}
		elsif ( !(-r $config->{dir}.'/plugins/settings/randslappy.conf') ) {
			$message = "Unable to read \002$config->{dir}/plugins/settings/randslappy.conf\002. Please make sure you have proper permission for this file.";
		}
		else {
			$message = "Unknown problem encountered reading \002$config->{dir}/plugins/settings/randslappy.conf\002.";
		}
