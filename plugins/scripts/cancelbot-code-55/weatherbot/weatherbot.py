...
def get_weather(lookup, destination):
    ...
    destination.command("say The weather for " + response["city"] + " in " + response["postalCode"] + " is " +
    response["condition"] + " the temp is " + response["tempF"] + "F/" + response["tempC"] + "C with " + response["windCondition"] +
    " and a relative humidity of " + response["humidity"] + " on " + response["forecastDate"])
